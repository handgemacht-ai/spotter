...
if command -v tmux &>/dev/null; then
  if tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
    ...
    CLAUDE_CMD="unset CLAUDECODE; SPOTTER_URL=\"$SPOTTER_URL\" SPOTTER_PROJECT_DIR=\"$WORKTREE_ROOT\" claude --plugin-dir \"$PLUGIN_DIR\" --dangerously-skip-permissions"
    ...
    # Make sure `claude` sees `.envrc` vars even when started via tmux.
