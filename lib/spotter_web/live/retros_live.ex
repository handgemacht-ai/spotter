defmodule SpotterWeb.RetrosLive do
  ...
  alias Spotter.Transcripts.{RetroItem, RetroSubmission, SessionPresenter}
  require Ash.Query
  ...
  defp load_submissions(%{assigns: %{current_project_id: nil}} = socket) do
    assign(socket, submissions: [])
  end

  defp load_submissions(%{assigns: %{current_project_id: project_id}} = socket) do
    submissions =
      RetroSubmission
  ...
  def render(assigns) do
    ...
    <div class="container" data-testid="retros-root">
      ...
      <div :if={@current_project_id && @submissions == []} class="empty-state">
      ...
      <div :if={!@current_project_id} class="empty-state">
        No project selected.
      </div>
    </div>
    """
